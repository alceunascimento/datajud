...
$PYTHON main.py coletar --tipo municipio --codigos 4106902 --tribunais TJPR
...
3. Seleciona tribunais + período (opcional) + processos por página (1000 / 5000 / 10000)
...
6. ★ ENRIQUECER TPU      → adiciona colunas TPU nos Parquets   (habilitado após parse)
   ...
   👨‍⚖ MAGISTRADOS TJPR  → baixa magistrados + unidades do TJPR (sempre disponível)
```
...
| Município | `orgaoJulgador.codigoMunicipioIBGE` | código IBGE digitado | vírgula ou CSV |
| Combinada | qualquer combinação | — | — |

> A aba **Município** inclui botão "🔎 Encontrar código do município" que abre a [página do IBGE](https://www.ibge.gov.br/explica/codigos-dos-municipios.php).
...
├── magistrados_tjpr_{ts}.parquet                 # magistrados titulares + substitutos TJPR
└── unidades_tjpr_{ts}.parquet                    # unidades judiciárias TJPR
```
...
Fonte: `https://portal.tjpr.jus.br/magistratura/api/listaCompleta`. Cada linha carrega `dt_referencia` (data da coleta) — magistrados mudam de comarca/entrância, use a data para análise intertemporal.

| Coluna | Tipo |
|--------|------|
| `dt_referencia` | VARCHAR (YYYY-MM-DD) |
| `comarca_sede_raw` | VARCHAR (não-null só em RMs) |
| `comarca_sede` | VARCHAR |
| `comarca_nome_raw` | VARCHAR |
| `comarca_nome` | VARCHAR |
| `comarca_id` | INTEGER |
| `unidade_nome` | VARCHAR |
| `magistrado_id` | INTEGER (só preenchido para substitutos) |
| `magistrado_nome` | VARCHAR |
| `perfil` | VARCHAR (`titular` ou `substituto`) |

### unidades_tjpr_{ts}.parquet

Mesma coleta; relaciona cada unidade judiciária à comarca/entrância/seção.
...
| `unidade_email` / `unidade_id_domus` | VARCHAR |
...
├── magistrados.py   # TJPR: fetch + flatten → magistrados/unidades Parquet
├── requirements.txt
├── docs/
...
**Throttling:** 500ms entre páginas + backoff `2^n` (capado em 60s) em 429/500/502/503/504 e erros de rede. 4xx (≠429) falham imediatamente.

**Timeout:** `REQUEST_TIMEOUT=300s` por POST (configurável em `config.py`). Páginas grandes (`page_size=10000`) podem demorar > 60s no servidor DataJud.
...
| Magistrados TJPR | `https://portal.tjpr.jus.br/magistratura/api/listaCompleta` | Nenhuma |
